...
* Make OpenSSH sshd able to use private host keys generated by dropbearconv.
  (Maybe it's already true, if it doesn't check the comment.)
* Make OpenSSH sshd able to use private host keys generated by dropbearkey.
  (Maybe it's already true, if it doesn't check the comment.)
